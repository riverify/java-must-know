...
1. 有一个有序表为 1, 5, 8, 11, 19, 22, 31, 35, 40, 45, 48, 49, 50，当二分查找的值为48的结点时，查找成功需要比较的次数是多少次（京东实习生招聘）
   4次
2. 使用二分法在序列1, 4, 6, 7, 15, 33, 39, 50, 64, 78, 75, 81, 89, 96 中查找元素81时，需要经过（4）次比较（美团点评校招）
3. 在已经的128个数组中二分查找一个数，需要比较的次数最多不超过多少次（北京易道博时社区）log2 128 = 7
